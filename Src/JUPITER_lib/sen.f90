  ...
  SUBROUTINE SEN_INI_DI(IOUT,NDEP,NPE,NPT,IPTR,PARNAM,DEPNAM)
    ...
    IF (DI_NDEP>0) THEN
      IF (DI_NDEP.NE.NDEP) THEN
        ...
        CALL UTL_STOP(' ')
      ENDIF
      IF (.NOT. ALLOCATED(DI_DEPNAM)) ALLOCATE(DI_DEPNAM(DI_NDEP))
      IF (.NOT. ALLOCATED(DI_XCOL)) ALLOCATE(DI_XCOL(DI_NDEP))
    ELSE
      WRITE(IOUT,120)
      KERR = KERR+1
      CALL UTL_STOP(' ')
    ENDIF
    IF (DI_NPAR>0) THEN
      IF (.NOT. ALLOCATED(DI_PARNAM)) ALLOCATE(DI_PARNAM(DI_NPAR))
      IF (.NOT. ALLOCATED(DI_XROW)) ALLOCATE(DI_XROW(DI_NPAR))
    ELSE
      WRITE(IOUT,120)
      KERR = KERR+1
      CALL UTL_STOP(' ')
