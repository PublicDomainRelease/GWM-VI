MODULE PARALLEL_PROCESSING
  ...
  PUBLIC :: AUTOSTOPRUNNERS, COMMAND, FNDISFIN, FNDISPAR, FNDISRDY, FNRUNDEP,  &
            ...
            SNAME, WAIT, USESLEEP, WAITRUNNERS ! Added WAITRUNNERS 6/21/2010 ERB
  ! Added for model status capability - ERB 12/5/2008
  PUBLIC :: STATUSFILE, ISTATFILETYPE
  !
  ...
  DOUBLE PRECISION              :: WAIT = 0.001D0, WAITRUNNERS = 0.001D0 ! 6/12/2010 ERB
  ! Added for model status capability - ERB 12/5/2008
  CHARACTER(LEN=200) :: STATUSFILE
  INTEGER :: ISTATFILETYPE
...
CONTAINS
  ...
  SUBROUTINE PLL_INI_RUNNER_POP(IOUNIT,LCIS,NCATS,NINSTRUCT,NMIFILE,NMOFILE,   &
    ...
    USE UTILITIES ! Added for model status capability - ERB 12/5/2008
    IMPLICIT NONE
    !
    !   Argument-list variables
    INTEGER,                                            INTENT(IN)  :: IOUNIT
    ...
    ! Added for model status capability - ERB 12/5/2008
    INTEGER :: ICOL, ISTART, ISTOP, NDUM
    REAL :: RDUM
    CHARACTER(LEN=200) :: LINE, OPT 
    !
    !   Format statements
    100 FORMAT(A)
    200 FORMAT(6(1X,A12))
    ...
    !
    !   Added for model status capability - ERB 12/5/2008
    !   Initialize variables that control model status capability
    STATUSFILE = ' '
    ISTATFILETYPE = 0
    !
    ...
    ! Add support for reading OPTION line(s) and model status file - ERB 12/5/2008
    READ(IOUNIT,100,END=30)LINE
    IF (LINE.NE.' ') THEN
      ICOL = 1
      CALL UTL_RWORD(-1,1,.FALSE.,ICOL,LINE,ISTART,ISTOP,NDUM,RDUM)
      OPT = LINE(ISTART:ISTOP)
      IF (OPT=='STATUSFILE') THEN
        CALL UTL_RWORD(-1,0,.FALSE.,ICOL,LINE,ISTART,ISTOP,NDUM,RDUM)
        STATUSFILE = LINE(ISTART:ISTOP)
        CALL UTL_RWORD(-1,2,.FALSE.,ICOL,LINE,ISTART,ISTOP,ISTATFILETYPE,RDUM)
      ENDIF
    ENDIF
    30 CONTINUE
    !
    ...
    K = 0
    50 CONTINUE
    ISTAT = PLL_CLOSE_AND_DELETE(IOUNIT,FNDISRDY)
    IF (ISTAT .NE. 0) THEN
      K = K+1
      IF (K<11) THEN
        CALL PLL_WAIT()
  ...
  SUBROUTINE PLL_MAKE_RUNS(LCIS,NCATS,NINSTRUCT,NMIFILE,NMOFILE,NOPNT,   &
                           ...
                           PARVALSETS,TFILE,DEPVALSETS,COMMANDPLL,OPTIONFLAG)
  ...
  SUBROUTINE PLL_RUNNER_STOP(IFLAG)
    ...
    !
    !   Local variables
    INTEGER :: IFL, ISTAT, IU, K10, K11, K20, K21, K40, K41, K50, K51
    LOGICAL :: LEX
    !
    !   Format statements
