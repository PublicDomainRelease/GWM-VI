...
CONTAINS
  ...
  SUBROUTINE JRN_INI()
    ...
    USE UTILITIES
    USE PARALLEL_PROCESSING, ONLY: FNDISFIN, FNDISPAR, FNRUNDEP, &
                                   FNRUNFAIL, FNRUNFIN, PLL_WAIT, &
                                   PLL_CLOSE_AND_DELETE
    IMPLICIT NONE
    !
    !   Local variables
    INTEGER :: IOUNIT, ISTAT, K, KTRY
    LOGICAL :: LEX
    !
    !   Format statements
    ...
    IF (LEX) THEN
      ...
      KTRY = 0
      30 CONTINUE
      ISTAT = PLL_CLOSE_AND_DELETE(IOUNIT,FNRUNFIN)
      IF (ISTAT>0 .AND. KTRY<11) THEN
        KTRY = KTRY+1
        CALL PLL_WAIT()
        GOTO 30
    ...
    IF (LEX) THEN
      ...
      KTRY = 0
      40 CONTINUE
      ISTAT = PLL_CLOSE_AND_DELETE(IOUNIT,FNRUNDEP)
      IF (ISTAT>0 .AND. KTRY<11) THEN
        KTRY = KTRY+1
        CALL PLL_WAIT()
        GOTO 40
    ...
    IF (LEX) THEN
      ...
      KTRY = 0
      50 CONTINUE
      ISTAT = PLL_CLOSE_AND_DELETE(IOUNIT,FNRUNFAIL)
      IF (ISTAT>0 .AND. KTRY<11) THEN
        KTRY = KTRY+1
        CALL PLL_WAIT()
        GOTO 50
    ...
    IF (LEX) THEN
      ...
      IF (ISTAT==0) THEN
        ISTAT = PLL_CLOSE_AND_DELETE(IOUNIT,FNDISPAR)
        IF (ISTAT.NE.0) THEN
          CALL PLL_WAIT()
          K = K+1
          IF (K>10) THEN
  ...
  !-----------------------------------------------------------------------------
  SUBROUTINE JRN_POPEXTVAL(MFSTAT,DWSTAT,HCLOSE,HDRY,HNOFLO)
    ! Populate EXTVAL with bogus dependent values and values 
    ! read from modflow.status.
    IMPLICIT NONE
    !   Argument-list variables
    INTEGER, INTENT(IN) :: MFSTAT, DWSTAT
    DOUBLE PRECISION, INTENT(IN) :: HCLOSE, HDRY, HNOFLO
    !   Local variable
    INTEGER :: I, ND
    ...
    EXTVAL(ND+2) = DWSTAT
    RETURN
  END SUBROUTINE JRN_POPEXTVAL
END MODULE JRUNNERMMOD